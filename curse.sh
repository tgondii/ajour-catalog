...
do
  ...
  echo $(echo "$data" | jq 'map({"id": .id, "website_url": .websiteUrl, "name": .name, "summary": .summary, "numberOfDownloads": .downloadCount, "categories": [.categories[] | .name], "source": "curse" })') > new.json
